...
@section('title', 'SER - Detalhe requisição ')
        ...
        <div class="col-lg-12">
            <div class="card card-secondary">
                <div class="card-header">
                    <h3 class="card-title"> {{$req_details->tag}} - <span class="badge p-2 @if($req_details->level_id == 2) badge-warning @endif">{{$req_details->requisition_level->name}}</span> </h3>

                </div>
                <form method="POST" action="{{url('/requisition-management/edit-pending/'. $req_details->id)}}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                <div class="card-body">
                    <div class="row">
